...
function makeFinding(rule, file, message, path2, line = 1) {
  return {
    ruleId: rule.id,
...
// src/scanner.js
var WORKFLOW_DIR = import_node_path.default.join(".github", "workflows");
var SHA_RE = /^[a-f0-9]{40,64}$/i;
var UNTRUSTED_RUN_EXPR_RE = /\$\{\{\s*(github\.event|github\.head_ref|github\.base_ref|inputs\.|matrix\.)/;
async function scanPath(targetPath = process.cwd(), options = {}) {
  const root = import_node_path.default.resolve(targetPath);
...
async function scanWorkflowFile(filePath, root = process.cwd(), options = {}) {
  const absoluteFile = import_node_path.default.resolve(filePath);
  const displayFile = import_node_path.default.relative(import_node_path.default.resolve(root), absoluteFile) || import_node_path.default.basename(absoluteFile);
  const source = await import_node_fs.promises.readFile(absoluteFile, "utf8");
  const lineMap = buildLineMap(source);
  let workflow;
  try {
    workflow = (0, import_yaml.parse)(source);
  } catch (error) {
    return [
      makeFinding(
...
async function findWorkflowFiles(root) {
  const workflowRoot = import_node_path.default.join(root, WORKFLOW_DIR);
  let entries;
  try {
    entries = await import_node_fs.promises.readdir(workflowRoot, { withFileTypes: true });
  } catch (error) {
  ...
  return entries.filter((entry) => entry.isFile() && /\.(ya?ml)$/i.test(entry.name)).map((entry) => import_node_path.default.join(workflowRoot, entry.name));
}
...
function shouldReportUnpinnedAction(usesValue, options) {
  return isExternalAction(usesValue) && !isPinnedToSha(usesValue) && (options.strict || !isTrustedFirstPartyAction(usesValue));
}
...
function checkPermissions(permissions, file, pathName, lineMap) {
  ...
  return Object.entries(permissions).filter(([scope, value]) => value === "write" && scope !== "id-token").map(
    ([scope]) => makeFinding(
      RULES.broadPermissions,
      file,
...
function buildLineMap(source) {
  ...
  const map = /* @__PURE__ */ new Map([["$", 1]]);
  const stack = [{ indent: -1, path: "$", arrayCounts: /* @__PURE__ */ new Map() }];
  lines.forEach((line, index) => {
    ...
    if (arrayItem) {
      ...
      const keyPath2 = `${itemPath}.${arrayItem[1]}`;
      map.set(itemPath, lineNumber);
      map.set(keyPath2, lineNumber);
      stack.push({ indent, path: itemPath, arrayCounts: /* @__PURE__ */ new Map() });
      return;
    }
    ...
    stack.push({ indent, path: keyPath, arrayCounts: /* @__PURE__ */ new Map() });
  });
  return map;
}
...
async function setOutput(name, value) {
  ...
  if (!file) return;
  await import_node_fs2.promises.appendFile(file, `${name}=${value}
`);
}
...
async function main() {
  ...
  await import_node_fs2.promises.writeFile(sarifFile, JSON.stringify(toSarif(findings), null, 2));
